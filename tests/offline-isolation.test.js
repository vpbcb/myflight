...
test('manual update preserves other PWAs and all user data on the shared origin', async () => {
    ...
    const context = vm.createContext({
        URL, console, checkRealInternet: async () => true,
        ...
        caches, window: { caches, location: { href: 'https://example.test/myflight/', pathname: '/myflight/' } }
    });
    vm.runInContext(resetSource, context);
    await vm.runInContext('hardResetApp()', context);
    assert.deepEqual(removed, ['myflight_v.1']);
    assert.deepEqual(unregistered, ['myflight']);
    assert.deepEqual(deletedKeys, ['hubActiveCacheVersion', 'hubVersion']);
});

test('manual update offline leaves everything untouched', async () => {
    let warned = false;
    const context = vm.createContext({ checkRealInternet: async () => false, alert: () => { warned = true; } });
    vm.runInContext(resetSource, context);
    await vm.runInContext('hardResetApp()', context);
...
test('worker fallback ignores another application cache even for matching URL', async () => {
    ...
    assert.equal(await vm.runInContext("get('./index.html')", context), 'own fallback');
